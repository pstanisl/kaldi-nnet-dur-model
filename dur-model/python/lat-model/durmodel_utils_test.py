...
class FeatureTests(unittest.TestCase):
    ...
    def test_syllabify(self):
        ...
        self.assertEquals(
            [['k', 'o'], ['mm', 'i']],
            durmodel_utils.syllabify(
                ['k', 'o', 'mm', 'i'], language='ESTONIAN',
                nonsilence_phonemes=set('k o mm i'.split())))
        self.assertEquals(
            [['a'], ['j', 'u']],
            durmodel_utils.syllabify(
                ['a', 'j', 'u'], language='ESTONIAN',
                nonsilence_phonemes='a j u'.split()))
        self.assertEquals(
            [['AH', 'B'], ['S', 'T', 'EY', 'N']],
            durmodel_utils.syllabify(
                'AH B S T EY N'.split(), language='ENGLISH',
    ...
    def _test_linear_lattice(self):
        ...
        seqs.append((
            1,
            '59_59_59_59_59_93_93_93_93_93_93_93_80_80_80_80_80_80_80_80_80_80_80_80_80_80_80_80_80_80_80'.split('_')))
        # SIL
        ...
        phone_map = {
            6: 'SIL', 13: 'a', 25: 'e', 59: 'k', 68: 'l', 93: 'o',
            80: 'mm', 85: 'n', 135: 't'}
        features_and_durs = []
    ...
    def test_compile_features_for_word(self):
        context = {
            ...
            -1: set([('fii', 1), ('dur', 0.553)]), +1: set([('</s>', 1)])}
        local_feature_and_dur_seq = [
            (set([('bii', 1), ('boo', 1)]), 5),
            (set([('buu', 1), ('bee', 1)]), 6)]
        ...
        self.assert_(
            ('pos-1:dur', durmodel_utils.dur_function(5)) in result[-1][0])

    def test_tedlium(self):
        transitions = durmodel_utils.read_transitions(
            dirname(__file__) + '/test_data/tedlium/transitions.txt')
        nonsilence_phonemes = set()
        for l in open(
                dirname(__file__) + '/test_data/tedlium/nonsilence.txt'):
            nonsilence_phonemes.add(l.partition('_')[0])

        word_list = []
        for l in codecs.open(
            dirname(__file__) + '/test_data/tedlium/words.txt',
                encoding='UTF-8'):
        ...
        features_and_dur_seq = durmodel_utils.make_local(
            0, word_id, frames, transitions, word_list, nonsilence_phonemes,
            language='ENGLISH', stress_dict=stress_dict)
        print(features_and_dur_seq)
    ...
    def test_stress(self):
        ...
        stress_dict = durmodel_utils.load_stress_dict(
            dirname(__file__) + '/data/en/cmudict.0.7a.lc')
        self.assertEqual(
            ['AH', 'B', 'S', 'T', 'EY', 'N'], stress_dict['abstain'][0][0])
        self.assertEqual(
            [0, 0, 0, 0, 1, 0], stress_dict['abstain'][0][1])
        stress = durmodel_utils.get_stress(word, phones, stress_dict)
        self.assertEqual([0, 0, 0, 0, 1, 0], stress)
        # test unknown word
        stress = durmodel_utils.get_stress(
            'tanel', 'T AH N EH L'.split(), stress_dict)
