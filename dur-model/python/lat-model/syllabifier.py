...
# syllables = syllabifier.syllabify(language, "AO2 R G AH0 N AH0 Z EY1 SH AH0
# N Z")
#
...
English = {
    ...
    'vowels': [
        ...
        'OY', 'UH', 'UW'],
    'onsets': [
        'P', 'T', 'K', 'B', 'D', 'G', 'F', 'V', 'TH', 'DH', 'S', 'Z', 'SH',
        'CH', 'JH', 'M', 'N', 'R', 'L', 'HH', 'W', 'Y', 'P R', 'T R', 'K R',
        'B R', 'D R', 'G R', 'F R', 'TH R', 'SH R', 'P L', 'K L', 'B L', 'G L',
        'F L', 'S L', 'T W', 'K W', 'D W', 'S W', 'S P', 'S T', 'S K', 'S F',
        'S M', 'S N', 'G W', 'SH W', 'S P R', 'S P L', 'S T R', 'S K R',
        'S K W', 'S K L', 'TH W', 'ZH', 'P Y', 'K Y', 'B Y', 'F Y', 'HH Y',
        'V Y', 'TH Y', 'M Y', 'S P Y', 'S K Y', 'G Y', 'HH W', '']
    }

Estonian = {
    'consonants' : 'f h j k l m n p r s sh t v ff hh jj kk ll mm nn pp rr ss shsh tt vv'.split(),
    'vowels' : 'a e i o u ou ae oe ue aa ee ii oo uu ouou aeae oeoe ueue'.split(),
    'onsets' : 'f h j k l m n p r s sh t v ff hh jj kk ll mm nn pp rr ss shsh tt vv'.split()
}
...
def loadLanguage(filename):
    ...
    for line in f:
        ...
        if line in ('[consonants]', '[vowels]', '[onsets]'):
            ...
            raise ValueError(
                'File must start with a section header such as [consonants].')
    ...
    for section in 'consonants', 'vowels', 'onsets':
        if len(L[section]) == 0:
            raise ValueError(
                'File does not contain any consonants, vowels, or onsets.')
...
def syllabify(language, word):
    ...
    loadLanguage. Word is either a string of phonemes from the CMU pronouncing
    dictionary set (with optional stress numbers after vowels), or a Python
    list of phonemes, e.g. "B AE1 T" or ["B", "AE1", "T"]'''
        ...
        if phoneme in language['vowels']:
            ...
            if '.' in internuclei:
                ...
                for split in range(0, len(internuclei)+1):
                    ...
                    # better than a coda that doesn't follow a nucleus),
                    # or if we've gone through all of the onsets and we
                    # didn't find any that are valid, then split the
                    # nonvowels we've seen at this location.
                    if ' '.join(onset) in language['onsets'] \
                       or len(syllables) == 0 \
                       or len(onset) == 0:
                        break

            # Tack the coda onto the coda of the last syllable. Can't do it if
            ...
            raise ValueError(
                'Invalid phoneme: ' + phoneme + ' in word ' + str(word))
        else:  # a consonant
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) != 2:
        print('Usage: python syllabifier.py ENGLISH < textfile.txt > outfile.txt')
    else:
